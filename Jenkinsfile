            ...
            steps {
                dir('infra') {
                    sh '''
                        ...
                        -target=docker_network.cicd \
                        -target=docker_image.sentiment \
                        -target=docker_container.sentiment_staging \
        ...
        stage('Smoke Test') {
            steps {
                sh '''
                    ...
                    --network cicd-network \
                    curlimages/curl:8.7.1 \
                    -fsS http://sentiment-staging:8000/metrics > /dev/null
                '''
                echo 'Smoke test réussi : endpoint /metrics accessible.'
            }
        }
    }
